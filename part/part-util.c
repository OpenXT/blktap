...
static int
set_signature(const char *image, long int sig)
{
	int fd, ret;
	struct partition_table pt;
...
/*
 * nabbed from vhd spec.  simulates vhd geometry.
 */
void
calculate_geometry(uint64_t size, struct hd_geometry *geo)
{
...
main(int argc, char *argv[])
{
	char *image;
	struct partition_table pt;
	int ret, c, type, count, dump, format, signature, write_sig, sig_to_write;
	...
	while ((c = getopt(argc, argv, "i:fdt:cw:s:h")) != -1) {
		switch (c) {
		case 'i':
			image = optarg;
			break;
	...
	if (!image || (!format && !count && !dump 
		       && (signature==-1) 
		       && (!write_sig && (sig_to_write==-1)))) {
		errno = EINVAL;
		usage(argv[0]);
		goto out;
	}
	...
	if (write_sig) {
		if (set_signature(image, sig_to_write)) {
			perror("writing signature");
			goto out;
		}
	}
